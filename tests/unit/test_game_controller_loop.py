...
class TestGameControllerLoop(unittest.TestCase):
    """Test unittest per il loop pubblico del game controller."""

    def setUp(self) -> None:
        from bingo_game.game_controller import avvia_partita_sicura, crea_giocatori_automatici, crea_partita_standard
        from bingo_game.partita import Partita
        from bingo_game.tabellone import Tabellone

        self.partita_avviata = crea_partita_standard(
            nome_giocatore_umano="TestPlayer",
            num_cartelle_umano=1,
            num_bot=1,
        )
        avvia_partita_sicura(self.partita_avviata)

        tabellone = Tabellone()
        bot_list = crea_giocatori_automatici(num_bot=2)
        self.partita_solo_bot = Partita(tabellone, bot_list)
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_ritorna_non_none(self) -> None:
        ...
        risultato = ottieni_giocatore_umano(self.partita_avviata)
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_nome_corretto(self) -> None:
        ...
        giocatore = ottieni_giocatore_umano(self.partita_avviata)
        self.assertIsNotNone(giocatore)
        self.assertEqual(giocatore.nome, "TestPlayer", f"Nome atteso 'TestPlayer', ottenuto '{giocatore.nome}'")
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_solo_bot_ritorna_none(self) -> None:
        """Con partita senza GiocatoreUmano deve ritornare None."""
        from bingo_game.game_controller import ottieni_giocatore_umano

        risultato = ottieni_giocatore_umano(self.partita_solo_bot)
        self.assertIsNone(risultato, "Atteso None con partita di soli bot")
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_none_ritorna_none(self) -> None:
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_intero_ritorna_none(self) -> None:
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_stringa_ritorna_none(self) -> None:
...
# ---------------------------------------------------------------------------
    ...
    def test_ottieni_giocatore_umano_nessun_side_effect(self) -> None:
        ...
        stato_prima = ottieni_stato_sintetico(self.partita_avviata)
        ottieni_giocatore_umano(self.partita_avviata)
        stato_dopo = ottieni_stato_sintetico(self.partita_avviata)

        self.assertEqual(stato_prima["stato_partita"], stato_dopo["stato_partita"])
        self.assertEqual(stato_prima["numeri_estratti"], stato_dopo["numeri_estratti"])
...
# ---------------------------------------------------------------------------
    ...
    def test_smoke_crea_partita_standard(self) -> None:
        ...
        self.assertEqual(partita.get_numero_giocatori(), 2)
...
# ---------------------------------------------------------------------------
    ...
    def test_smoke_esegui_turno_sicuro(self) -> None:
        ...
        risultato = esegui_turno_sicuro(self.partita_avviata)
...
# ---------------------------------------------------------------------------
    ...
    def test_smoke_ottieni_stato_sintetico(self) -> None:
        ...
        stato = ottieni_stato_sintetico(self.partita_avviata)
